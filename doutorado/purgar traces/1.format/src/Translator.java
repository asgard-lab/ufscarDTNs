...
public class Translator {
	...
	Map<Integer, ArrayList<Object>> busesActivityTimes = null;
	...
	private void cutByTime() throws Exception {
	...
	private void convertTimeToSeconds() {
		...
		// AGREGADO
		for (Map.Entry<Integer, ArrayList<Object>> entry : this.busesActivityTimes
				.entrySet()) {

			ArrayList<Object> tmp = entry.getValue();
			int listDatesSize = tmp.size();
			ArrayList<Object> dtmp = new ArrayList<Object>();
			for (int i = 0; i < listDatesSize; i++) {
				Date dateTmp = (Date) tmp.get(i);
				dtmp.add((dateTmp.getTime() / (double) (1000)) - minDate);

			}
			// tmp.clear();
			// tmp.addAll(dtmp);
			this.busesActivityTimes.put(entry.getKey(), dtmp);
		}
	...
	private void arrangeAreaTime() {
		// TODO Auto-generated method stub
		List<ArrayList<Object>> tmpData = new ArrayList<ArrayList<Object>>();
		Map<Integer, ArrayList<Object>> busesActivity = null;
		busesActivity = new HashMap<Integer, ArrayList<Object>>();
		for (int k = 0; k < this.data.size(); k++) {
			ArrayList<Object> e = data.get(k);
			Date eDate = (Date) e.get(0);
			int idBus = (int) e.get(1);
			...
			if (!busesActivity.containsKey(idBus)) {
				busesActivity.put(idBus, new ArrayList<Object>());
			}
			int sizeBusesActivity = (busesActivity.get(idBus)).size();
			if (sizeBusesActivity == 0 ) {
				busesActivity.get(idBus).add(eDate);
			...
			else
			{
				if (busesActivity.containsKey(idBus)) {
					...
					if (sizeBusesActivity  == 1)
						busesActivity.get(idBus).add(eDate);
					else 
					{
						if (sizeBusesActivity  == 2)
							busesActivity.get(idBus).remove(1);
							busesActivity.get(idBus).add(eDate);
					}
				}
			}
		...
		this.busesActivityTimes=busesActivity;
	}



	private void updateSpaceTimeLimits()  {
	...
	private void createCoordenadesFile(Set<Point> coordenades)
		...
		try {
			...
			bw.append(cadena);
			bw.newLine();
			cadena = "highspeedInterface.transmitSpeed = 0.0625M";
			bw.append(cadena);
			bw.newLine();
			cadena = "highspeedInterface.transmitRange = 250";
			bw.append(cadena);
			bw.newLine();
			cadena = "Group.bufferSize = 2G";
			bw.append(cadena);
			bw.newLine();
			cadena = "Group.waitTime = 0, 50";
			bw.append(cadena);
			bw.newLine();
	...
	public void getActiveFile() throws IOException {
		...
		String linea = "";
		int count = 0;
		Map <Integer,Double> tmpBusesFinalTime= new HashMap<Integer, Double>();
		try {
			while (((linea = br.readLine()) != null) /* && (contador <= 100000) */) {
				if (count > 0) {
					...
					bw.append(output);
					bw.newLine();
					if(!tmpBusesFinalTime.containsKey(id))
						tmpBusesFinalTime.put(id, Double.parseDouble(endTime));
			...
			br.close();
			fr.close();
			FileWriter wrDetail = new FileWriter(this.pathAndFileInfo.get(1)
					+ "active\\" + this.pathAndFileInfo.get(0)
					+ "activeFileDetail.txt");

			BufferedWriter bwDetail = new BufferedWriter(wrDetail);

			// AGREGADO
			// this.busesIds.put(busid, contBuses);
			int size = this.busesIds.size();
			List<Integer[]> realIds = new ArrayList<Integer[]>();

		

			for (Map.Entry<Integer, Integer> entry : this.busesIds.entrySet()) {
				// ArrayList<Object> tmp = new ArrayList<Object>();
				Integer[] tmp = { entry.getValue(), entry.getKey() };
				realIds.add(tmp);
			...
			Collections.sort(realIds, new Comparator<Integer[]>() {
				@Override
			...
			for (int i = 0; i < size; i++) {
				int anteriorId = (int) realIds.get(i)[1];
				int realId = (int) realIds.get(i)[0];
				if (this.busesActivityTimes.containsKey(anteriorId)) {
					ArrayList<Object> tmp = busesActivityTimes
							.remove(anteriorId);
					int listDatesSize = tmp.size();
					int idBusKey = realId;
					String activity = "" + idBusKey;
					for (int j = 0; j < listDatesSize; j++) {

						double dDate = (double) tmp.get(j);
						if ((listDatesSize % 2) == 0 && (j==(listDatesSize-1)))
							continue;
						activity = activity + " " + dDate;
					}

					if (listDatesSize > 0 ) {
															
						
						if(tmpBusesFinalTime.containsKey(idBusKey))
						activity = activity + " " + tmpBusesFinalTime.get(idBusKey);
					}

					bwDetail.append(activity);
					bwDetail.newLine();
				}
			}
			bwDetail.close();
