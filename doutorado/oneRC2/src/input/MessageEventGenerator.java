...
public class MessageEventGenerator implements EventQueue {
	...
	// agregado://///////////////////////////////////////////////////////////////////
	public static final String HOST_IDS_LIST_S = "hostIdsList";
	public static final String RANDOM_SEED_ID_FROM_LIST_S = "hostIdSeed";
	private int[] posibleToNodes;
	protected Random rngSourcesAndDestinies;
	int seed;
	private static final int def_seed= 868955673;
	...
	public MessageEventGenerator(Settings s) {
		...
		// agregado/////////////////
		if (s.contains(HOST_IDS_LIST_S)) {
			this.posibleToNodes = s.getCsvInts(HOST_IDS_LIST_S);
		} else {
			this.posibleToNodes = null;
		}
		
		if (s.contains(RANDOM_SEED_ID_FROM_LIST_S)) {
			this.seed = s.getInt(RANDOM_SEED_ID_FROM_LIST_S);
		} else {
			this.seed = this.def_seed;
		}
		this.rngSourcesAndDestinies= new Random(this.seed);
		// ////////////////////////

		/* calculate the first event's time */
		this.nextEventsTime = (this.msgTime != null ? this.msgTime[0] : 0)
				+ msgInterval[0]
				+ (msgInterval[0] == msgInterval[1] ? 0 : rng
						.nextInt(msgInterval[1] - msgInterval[0]));
	}
	...
	protected int drawMessageSize() {
		int sizeDiff = sizeRange[0] == sizeRange[1] ? 0 : rng
				.nextInt(sizeRange[1] - sizeRange[0]);
	...
	protected int drawNextEventTimeDiff() {
		int timeDiff = msgInterval[0] == msgInterval[1] ? 0 : rng
				.nextInt(msgInterval[1] - msgInterval[0]);
