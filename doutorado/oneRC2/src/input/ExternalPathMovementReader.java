...
/**
 * <p>
 * External movement reader for traces that are in path format. Uses two trace
 * files, one for the paths and one for specifying activity times. Nodes will
 * follow the paths in the trace file, and pause between paths. Activity times
 * refer to the periods of time when there is valid trace data about the node.
 * </p>
 * 
 * <p>
...
public class ExternalPathMovementReader {
	...
	private static Map<String, ExternalPathMovementReader> singletons = new HashMap<String, ExternalPathMovementReader>();
	...
	private ExternalPathMovementReader(String traceFilePath,
		...
		try {
			if (traceFilePath.endsWith(".gz") || traceFilePath.endsWith(".zip")
		...
		/*
		 * Scanner scanner = null; try { scanner = new Scanner(inFile); } catch
		 * (FileNotFoundException e) { throw new
		 * SettingsError("Couldn't find external movement input " + "file " +
		 * inFile); }
		 */
		...
		line = reader.readLine();
		while (line != null) {
			// Aqui teria que mudar para tirar atividade de radio
			/*
			 * ISTO TINHA ANTES, EU REEMPRAZEI PELO MODIFICADO
			 * 
			 * Scanner traceScan = new Scanner(line); int id =
			 * traceScan.nextInt(); double start = traceScan.nextDouble();
			 * double end = traceScan.nextDouble(); List<ActiveTime> times =
			 * this.activeTimes.get(id); ActiveTime a = new ActiveTime();
			 * a.start = start; a.end = end; if (this.normalize) { a.start -=
			 * this.minTime; a.end -= this.minTime; } times.add(a);
			 */
			/* AQUI O MODIFICADO: */
			StringTokenizer traceScan = new StringTokenizer(line);
			int id = Integer.parseInt(traceScan.nextToken());
			if (traceScan.countTokens() % 2 == 0) {
				List<ActiveTime> times = this.activeTimes.get(id);
				while (traceScan.hasMoreTokens()) {
					double start = Double.parseDouble(traceScan.nextToken());
					double end = Double.parseDouble(traceScan.nextToken());
				...
				throw new SettingsError("Error in activity times file. We need a pair number of times (start, end)");
			}
	 ...
	 * 
	 * @param filePath
	 *            path where the file is read from
