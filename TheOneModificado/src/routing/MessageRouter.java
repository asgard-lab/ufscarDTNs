...
public abstract class MessageRouter {
	 ...
	 * message can be sent directly to the final recipient). Valid values are<BR>
	 * <UL>
	...
	/*
	 * Return values when asking to start a transmission: RCV_OK (0) means that
	 * the host accepts the message and transfer started, values < 0 mean that
	 * the receiving host will not accept this particular message (right now),
	 * values > 0 mean the host will not right now accept any message. Values in
	 * the range [-100, 100] are reserved for general return values (and
	 * specified here), values beyond that are free for use in implementation
	 * specific cases
	 */
	...
	Map<String, MutableInt> abortedMessages;
	...
	public void init(DTNHost host, List<MessageListener> mListeners) {
		...
		this.abortedMessages = new HashMap<String, MutableInt>();
		this.host = host;
	}
	...
	protected Message getMessageFromIncomingBuffer(String id) {
	 ...
	 *         bigger than zero if the other node should try later (e.g.
	 *         TRY_LATER_BUSY).
	 */
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		//removeFromAbortedBuffer(id, from);
	 ...
	 *            The message to put
	 * @param from
	 *            Who the message was from (previous hop).
	 */
	...
	/**
	 * Puts a message to aborteeed messages buffer. Two messages with the same
	 * ID are distinguished by the from host.
	 * 
	 * @param m
	 *            The message to put
	 * @param from
	 *            Who the message was from (previous hop).
	 */
	protected void putToAbortedBuffer(String id, DTNHost from) {

		MutableInt count = this.abortedMessages.get(id + "_" + from.toString());
		if (count == null) {
			this.abortedMessages.put(id + "_" + from.toString(),
					new MutableInt());
		} else {
			count.increment();
		}
	...
	/**
	 * Removes and returns a message with a certain ID from the aborted messages
	 ...
	 */
	protected MutableInt removeFromAbortedBuffer(String id, DTNHost from) {
		return this.abortedMessages.remove(id + "_" + from.toString());
	}
	 ...
	 * @return True if such message is incoming right now
	 */
	protected MutableInt isAbortedMessage(String id) {

		MutableInt count = this.abortedMessages.get(id );
	 ...
	 *            The message to add
	 * @param newMessage
	 *            If true, message listeners are informed about a new message,
	 *            if false, nothing is informed.
	 */
	 ...
	 * 
	 * @param id
	 *            Id of the message that was being transferred
	 * @param from
	 *            Host the message was from (previous hop)
	...
	public void messageAborted(String id, DTNHost from, int bytesRemaining) {
		...
		//this.putToAbortedBuffer(id, from);
		for (MessageListener ml : this.mListeners) {
	...
	public void deleteMessage(String id, boolean drop) {
		...
		if (removed == null)
			throw new SimError("no message for id " + id + " to remove at "
					+ this.host);
	 ...
	 * 
	 * @param app
	 *            The application to attach to this router.
	 */
	public void addApplication(Application app) {
		if (!this.applications.containsKey(app.getAppID())) {
			this.applications
					.put(app.getAppID(), new LinkedList<Application>());
		}
	 ...
	 * 
	 * @param ID
	 *            The application ID or <code>null</code> for all apps.
	...
	public List<MessageListener> getMListener() {
