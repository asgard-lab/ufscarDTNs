...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	public static final String INDIVIDUAL_ACTIVITY_FILE = "activityFile";
	...
	public NetworkInterface(Settings s) {
		...
		this.activityFile = s.getSetting(INDIVIDUAL_ACTIVITY_FILE);
	}
	 ...
	 * 
	 * @param host
	 *            The host where the network interface is
	 */
	 ...
	 * 
	 * @param s
	 *            The settings object using the right group namespace
	 */
	...
	/**
	 * Sets node-based settings for the network interface
	 * 
	 * @param s
	 *            The settings object using the right group namespace
	 */
	public void setNodeSettings(Settings s, String intervals) {

		ah = new ActivenessHandler(s, intervals);
	...
	public boolean isActive() {
		...
		if (active
				&& host.getComBus().getDouble(EnergyModel.ENERGY_VALUE_ID, 1) <= 0) {
		...
		if (active == false && this.transmitRange > 0) {
			...
			host.getComBus().updateProperty(RANGE_ID, this.oldTransmitRange);
		}
		return active;
	}
	...
	public boolean isScanning() {
		...
		/*
		 * interval == 0 or still in the same scan round as when last time asked
		 */
		return true;
	}
	 ...
	 * 
	 * @param anotherInterface
	 *            The other host's network interface to disconnect from this
	 *            host
	 */
	 ...
	 * @param anotherInterface
	 *            The interface of the other host
	 */
	...
	public Queue<Double> getInitActiveTimes() {
		return ah.getInitTimes();
	}
	public Queue<Double> getEndActiveTimes() {
		return ah.getEndTimes(); 
