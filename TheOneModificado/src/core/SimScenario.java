...
public class SimScenario implements Serializable {
	...
	/** simulate individualActivityHandler -setting id ({@value} ) */
	public static final String SIM_INDIVIDUAL_ACTIVITY = "individualActivity";
	public static final String INDIVIDUAL_ACTIVITY_CAPABILITY = "indActivityCapability";
	private int nroHostClassifiers = 0;
	/** The world instance */
	private World world;
	...
	private boolean individualActivity;
	private boolean indActivityCapability;
	...
	protected List<HashMap<String, String>> intervals;
	...
	protected SimScenario() {
		...
		if (s.contains(SIM_INDIVIDUAL_ACTIVITY)) {
			this.individualActivity = s.getBoolean(SIM_INDIVIDUAL_ACTIVITY);
		} else {
			this.individualActivity = false;
		}
		...
		createHosts();
		System.out.println(this.nroHostClassifiers);
	...
	protected void createHosts() {
		...
		for (int i = 1; i <= nrofGroups; i++) {
			...
			// for scenarios with radio ativity specifications
			if (s.contains(this.INDIVIDUAL_ACTIVITY_CAPABILITY)) {
				this.indActivityCapability = s
						.getBoolean(INDIVIDUAL_ACTIVITY_CAPABILITY);
				if (this.individualActivity == true)
					intervals = new ArrayList<HashMap<String, String>>();
			} else {
				indActivityCapability = false;
			}
			...
			for (int j = 1; j <= nrofInterfaces; j++) {
				...
				NetworkInterface iface = (NetworkInterface) intSettings
						.createIntializedObject(INTTYPE_PACKAGE
								+ intSettings.getSetting(INTTYPE_S));
				iface.setClisteners(connectionListeners);
				if (indActivityCapability == false)
					iface.setGroupSettings(s);
				else {
					if (this.indActivityCapability == true) {
						try {
							int nroActivityLines = readActivityRadioFile(iface.activityFile);
							if (nrofHosts != nroActivityLines) {
								throw new SimError(
										"There is not activity intervals for each node ");
							}
						} catch (SimError e) {
			...
			for (int j = 1; j <= appCount; j++) {
				...
				try {
					...
					protoApp = (Application) t
							.createIntializedObject(APP_PACKAGE
			...
			for (int j = 0; j < nrofHosts; j++) {
				...
				if (this.indActivityCapability == true) {
					for (int k = 1; k <= nrofInterfaces; k++) {
						HashMap<String, String> tmpHash = intervals.get(k - 1);
						String intervalsPerID = tmpHash.get(String.valueOf(j));
						interfaces.get(k - 1)
								.setNodeSettings(s, intervalsPerID);
					}
				}
				DTNHost host = null;
				...
				if (mRouterProto instanceof SaWClassifiedRouter
						|| mRouterProto instanceof EpidemicJ48Router) {
				...
				hosts.add(host);
				if (host.isClassifierMode())
					this.nroHostClassifiers++;
			}
		}
	}

	private int readActivityRadioFile(String activityFile) throws IOException {

		// TODO Auto-generated method stub
		HashMap<String, String> intervalosId = new HashMap<String, String>();
		FileReader reader = new FileReader(activityFile);
		BufferedReader bufferIn = new BufferedReader(reader);
		StringTokenizer tokens;
		String activityLine;
		int numReader = 0;
		while ((activityLine = bufferIn.readLine()) != null) {
			tokens = new StringTokenizer(activityLine);
			String ID = tokens.nextToken();
			String intervals = tokens.nextToken();
			intervalosId.put(ID, intervals);
			numReader++;

		}
		this.intervals.add(intervalosId);
		return numReader;
