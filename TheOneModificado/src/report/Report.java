...
public abstract class Report {
	...
	/**
	 * The interval (simulated seconds) of creating new settings files -setting
	 * id ({@value} )
	 */
	...
	/**
	 * Precision of formatted double values - setting id ({@value} ). Defines
	 * the amount of decimals shown in formatted double values. Default value is
	 * {@value #DEF_PRECISION}.
	 */
	...
	protected int x_size = 0;
	protected int y_size = 0;
	protected double endTime = 0;

	/**
	 * Constructor. Looks for a className.output setting in the Settings and if
	 * such is found, uses that as the output file name. Otherwise
	...
	/**
	 * Initializes the report output. Method is called in the beginning of every
	 * new report file. Subclasses must call this method first in their own
	...
	private void createSuffixedOutput(String outFileName) {
		String suffix = String
				.format(INTERVALLED_FORMAT, this.lastOutputSuffix);
	 ...
	 * logs. If the report has no meaningful use for multiple reports, the call
	 * can be omitted (then only single output file will be generated)
	 */
	...
	protected void writeNext(String txt) {
		if (out == null) {
			init();
		}
	...
	/**
	 * Called when the simulation is done, user requested premature termination
	 * or intervalled report generating decided that it's time for the next
	 * report.
	 */
	public void done() {
		if (out != null) {
	 ...
	 * @param values
	 *            The list of double values
	 * @return median of double values stored in the List or "NaN" for empty
	 *         lists.
	 */
