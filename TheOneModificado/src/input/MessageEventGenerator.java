...
public class MessageEventGenerator implements EventQueue {
	...
	protected Random rngSourcesAndDestinies=new Random(868955673);
	private List<Integer> posibleToNodes=new ArrayList<Integer>();
	...
	public MessageEventGenerator(Settings s){
		...
		this.posibleToNodes.add(4);
		this.posibleToNodes.add(6);
		this.posibleToNodes.add(10);
		this.posibleToNodes.add(13);
		this.posibleToNodes.add(20);
		this.posibleToNodes.add(33);
		this.posibleToNodes.add(37);
		this.posibleToNodes.add(44);
		this.posibleToNodes.add(50);
		this.posibleToNodes.add(59);
		this.posibleToNodes.add(62);
		this.posibleToNodes.add(607);
		this.posibleToNodes.add(618);
	...
	@Override
	public ExternalEvent nextEvent(List<DTNHost> hosts) {
		// TODO Auto-generated method stub
	//	int responseSize = 150; /* zero stands for one way messages */
		int responseSize = 0; /* zero stands for one way messages */
		int msgSize;
		int interval;
		int from;
		...
		/* Get two *different* nodes randomly from the host ranges */
		from = getpossibleFrom(hosts);	
		to = getpossibleTo(hosts, from);
	...
	private int getpossibleFrom(List<DTNHost>l)
	{
		
		int max=this.posibleToNodes.size()-1;
		
		
		int from=(int) (rngSourcesAndDestinies.nextDouble()*(max+1));
		//int from= min + (int)(Math.random() * ((max - min) + 1));
		return this.posibleToNodes.get(from);
		
		
	}
	private int getpossibleTo(List<DTNHost>l, int from)
	{
		...
		do
		{
		 int index=(int) (rngSourcesAndDestinies.nextDouble()*(max+1));
		 to=this.posibleToNodes.get(index);
		}
		while(to==from);
