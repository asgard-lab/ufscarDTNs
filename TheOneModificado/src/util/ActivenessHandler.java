...
/**
 * Object of this class tell the models when a node belonging to a certain group
 * is active and when not.
 */
public class ActivenessHandler {
	 ...
	 * must be in order and must not overlap.
	 * </P>
	 * <P>
	 ...
	 * </P>
	 * <P>
	 * Syntax: <CODE>activeTime, inactiveTime</CODE>
	 * </P>
	 * <P>
	 * Defines the activity and inactivity periods.
	 * </P>
	 * <P>
	 * Example: 2000,500<BR>
	 ...
	 * Default = 0 (i.e., first active period starts at 0)
	 * </P>
	 */
	...
	private Queue<Double> timesListIni = new LinkedList<Double>();
	private Queue<Double> timesListEnd = new LinkedList<Double>();
	...
	// LPPP
	public ActivenessHandler(Settings s, String intervals) {
		this.activeTimes = parseActiveTimes(s, intervals);
	...
	public Queue<Double> getInitTimes() {
		return this.timesListIni;

	}
	public Queue<Double> getEndTimes() {
		return this.timesListEnd;

	}

	private Queue<TimeRange> parseActiveTimes(Settings s, String intervals) {
		double[] times;

		times = s.parseDoubles(getCsvIntervals(intervals), "activeTimes");
		// .getDoublesInterval(ACTIVE_TIMES_S);
		if (times.length % 2 != 0) {
			throw new SettingsError("Invalid amount of values (" + times.length
					+ ") for setting " + intervals + ". Must "
					+ "be divisable by 2");
		}
		...
		for (int i = 0; i < times.length; i += 2) {
			double start = times[i];
			double end = times[i + 1];
			this.timesListIni.add(start);
			this.timesListEnd.add(end);
			if (start > end) {
				throw new SettingsError("Start time (" + start + ") is "
						+ " bigger than end time (" + end + ") in setting "
						+ intervals);
	...
	public String[] getCsvIntervals(String intervals) {
		ArrayList<String> values = new ArrayList<String>();

		Scanner s = new Scanner(intervals);
		s.useDelimiter(",");
	...
	public boolean isActive(int offset) {
		if (this.activeTimes == null) {
			if (this.activePeriods == null) {
				...
				int timeIndex = (SimClock.getIntTime()
						+ this.activePeriodsOffset + offset)
